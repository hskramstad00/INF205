...
IncidenceGraph::out_edges(const std::string& node) const {
    ...
    const Node* n = find_node_(node);
    if (!n) return result;
    for (const Edge* e : n->out)
        result.emplace_back(e->label, e->to->label);
    return result;
}
...
IncidenceGraph::successors(const std::string& node,
    ...
    if (!n) return result;
    for (const Edge* e : n->out)
        if (e->label == edge_label) result.push_back(e->to->label);
    return result;
}
...
IncidenceGraph::IncidenceGraph(const IncidenceGraph& other) {
    ...
    for (const auto& np : other.nodes_) {
        ...
        Node* nn = nodes_.back().get();
        by_label_[nn->label] = nn;
        map[np.get()] = nn;
    }
